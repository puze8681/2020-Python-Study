...
def infix2postfix(tokens):
    ...
    for token in tokens:
        ...
        # 토큰이 연산자라면
            ...
            while len(operators) and operators[len(operators) - 1] != "(" and precedence(token) < precedence(
...
def postfix2infix(postfix):
    ...
    for token in postfix:
        ...
        # 토큰이 연산자라면
            ...
            # operators 의 마지막 값을 지우고 그것을 right 라 선언
            right = values[len(values) - 1]
            del values[len(values) - 1]
            # operators 의 마지막 값을 지우고 그것을 left 라 선언
            left = values[len(values) - 1]
            del values[len(values) - 1]
            # values 의 끝에 left 를 right 로 연산해준 값을 추가
            values.append(calculate(left, right, token))
    # values 의 첫번쨰 값이 수식의 결과 값
    return values[0]


inputString = input('수식이 포함된 문자열을 입력해주세요. ')
token = tokenizing(inputString)
postfix = infix2postfix(token)
infix = postfix2infix(postfix)
print("{}\n{}".format(postfix, infix))
